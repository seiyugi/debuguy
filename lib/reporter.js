...
Reporter.prototype.parseLine = function(line) {
  ...
  // XXX: After defining better log format,
  // we can avoid using data.length to decide the existence of type and timestamp.
  if(data.length === 2){
    return {
    ...
    return {
      'time': data.splice(0, 1)[0],
      'type': data[data.length - 2].trim(),
      'timeStamp': parseInt(data[data.length -1], 10),
      'path': data[0].split('.')
    };
  }
};
